// File: approve_requests.js

class ApproveRequestManager {
    constructor() {
        this.currentPage = 1;
        this.itemsPerPage = 8;
    ...
    bindModalEvents() {
        ...
        // Form events
        const approverRole = document.getElementById('approverRole');
        if (approverRole) {
            approverRole.addEventListener('change', () => {
    ...
    applySorting() {
        ...
        this.currentRequests.sort((a, b) => {
            const tatA = this.calculateTAT(a.RequestDate || a.request_date || a.created_at);
            const tatB = this.calculateTAT(b.RequestDate || b.request_date || b.created_at);
    ...
    async loadRequests() {
        ...
        try {
            ...
            // Chỉ filter theo status, KHÔNG filter theo user
            ...
            // Build URL - LOAD TẤT CẢ REQUEST, KHÔNG RÀNG BUỘC USER
            if (this.currentFilter === 'all') {
                url = '/repair_requests';
            } else {
                url = '/repair_requests/by_Status';
            }
            ...
            this.currentRequests = Array.isArray(data) ? data : [];
            ...
            this.showAlert('Không thể tải danh sách yêu cầu: ' + error.message, 'danger');
            this.currentRequests = [];
        } finally {
            this.isLoading = false;
            ...
            paginatedRequests.forEach(request => {
                ...
                switch(request.Status) {
                    case 'PendingApproval':
                        statusBadge = '<span class="status-badge status-pendingapproval"><i class="fas fa-clock mr-1"></i> Chờ duyệt</span>';
                        break;
                    case 'Quoted':
                        statusBadge = '<span class="status-badge status-quoted"><i class="fas fa-file-invoice-dollar mr-1"></i> Đã báo giá</span>';
                        break;
                    case 'Approved':
                        statusBadge = '<span class="status-badge status-approved"><i class="fas fa-check-circle mr-1"></i> Đã duyệt</span>';
                        break;
                    case 'Rejected':
                        statusBadge = '<span class="status-badge status-rejected"><i class="fas fa-times-circle mr-1"></i> Từ chối</span>';
                        break;
                    default:
                        statusBadge = `<span class="status-badge bg-gray-600">${request.Status}</span>`;
                }
                
                // Kiểm tra xem có thể duyệt không - theo workflow EM -> LM
                ...
                <tr class="table-row-hover border-b border-gray-700/50">
                    <td class="px-6 py-4">
                        <span class="font-mono font-bold text-blue-300">YC-${request.id.toString().padStart(3, '0')}</span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="font-medium">${request.DeviceName || 'Thiết bị ' + request.DeviceID}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <i class="fas fa-user text-gray-400"></i>
                            <span>${request.RequesterName || 'User ' + request.RequestedBy}</span>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex gap-2">
                            <button class="view-request p-2 bg-blue-600 hover:bg-blue-500 rounded-lg text-white transition transform hover:scale-110" data-id="${request.id}" title="Xem chi tiết">
    ...
    canUserApproveRequest(request) {
        // Chỉ cho phép duyệt request có status PendingApproval
        if (request.Status !== 'PendingApproval') {
            return false;
        }
        
        // Load thông tin approvals để xác định bước tiếp theo
        return this.getNextApprovalRole(request.id) !== null;
    }

    // Xác định role tiếp theo cần phê duyệt
    getNextApprovalRole(requestId) {
        const request = this.currentRequests.find(r => r.id == requestId);
        if (!request) return null;
        
        // Giả sử chúng ta có thông tin approvals từ API
        // Nếu chưa có EM approval -> cần EM
        // Đã có EM approval và approved -> cần LM  
        // Đã có EM approval và rejected -> không cần tiếp (request bị reject)
        
        // Tạm thời trả về EM cho tất cả (sẽ được cập nhật khi load approval history)
        return 'EM';
    }
        ...
        const detail = `
                ...
                <div class="bg-gray-700/50 rounded-xl p-6 border border-gray-600">
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center py-2 border-b border-gray-600">
                            ...
                            <span class="text-white font-bold text-lg">YC-${request.id.toString().padStart(3, '0')}</span>
                        </div>
                        <div class="flex justify-between items-center py-2 border-b border-gray-600">
                            <span class="text-gray-400 font-medium">Thiết bị:</span>
                            <span class="text-white">${request.DeviceName || 'Thiết bị ' + request.DeviceID}</span>
                        </div>
                        <div class="flex justify-between items-center py-2 border-b border-gray-600">
                            <span class="text-gray-400 font-medium">Lab:</span>
                            <span class="text-white">${request.LabName || 'N/A'}</span>
                        </div>
                        <div class="flex justify-between items-center py-2">
                            <span class="text-gray-400 font-medium">Team:</span>
                            <span class="text-white">${request.TeamName || 'N/A'}</span>
                        </div>
                    </div>
                </div>
                <div class="bg-gray-700/50 rounded-xl p-6 border border-gray-600">
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center py-2 border-b border-gray-600">
                            ...
                            <span class="text-white">${request.RequesterName || 'User ' + request.RequestedBy}</span>
                        </div>
                        <div class="flex justify-between items-center py-2 border-b border-gray-600">
            ...
            <div class="bg-gray-700/50 rounded-xl p-6 border border-gray-600 mt-6">
                ...
                <div class="bg-gray-600/50 rounded-lg p-4">
                    <p class="text-gray-300 leading-relaxed">${request.Description || 'Không có mô tả'}</p>
                </div>
            </div>
        `;
    ...
    openApprovalModal(requestId) {
        ...
        this.setModalContent('requestId', `YC-${request.id.toString().padStart(3, '0')}`);
        this.setModalContent('requestDevice', request.DeviceName || 'Thiết bị ' + request.DeviceID);
        this.setModalContent('requestLab', request.LabName || 'N/A');
        this.setModalContent('requestRequester', request.RequesterName || 'User ' + request.RequestedBy);
        this.setModalContent('requestDate', this.formatDate(request.RequestDate));
        this.setModalContent('requestDescription', request.Description || 'Không có mô tả');
        ...
        if (approvalWarning) approvalWarning.classList.add('hidden');
        
        // Load lịch sử phê duyệt và cập nhật form
    ...
    async loadApprovalHistory(requestId) {
        try {
            const response = await fetch(`/approvals?RequestID=${requestId}&Step=RequestApproval`);
    ...
    // Cập nhật form phê duyệt dựa trên workflow
    updateApprovalForm() {
        ...
        // Xác định role tiếp theo cần phê duyệt
        const nextRole = this.determineNextApprovalRole();
        
        if (nextRole) {
            // Set role mặc định và disable select
            roleSelect.value = nextRole;
            roleSelect.disabled = true;
            
            // Hiển thị cảnh báo workflow
            if (warningElement) {
                if (nextRole === 'LM') {
                    ...
                    warningElement.innerHTML = `
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h5 class="font-semibold text-blue-400">Bước tiếp theo: Lab Manager</h5>
                                <p class="text-blue-300 text-sm mt-1">EM đã phê duyệt. Bạn đang thực hiện phê duyệt với vai trò LM.</p>
                            </div>
                        </div>
                    `;
                } else {
            ...
            // Cho phép chọn role nếu không xác định được next role
            roleSelect.disabled = false;
            if (warningElement) warningElement.classList.add('hidden');
        }
    }

    // Xác định role tiếp theo cần phê duyệt theo workflow
    determineNextApprovalRole() {
        const emApproval = this.currentApprovals['EM'];
        const lmApproval = this.currentApprovals['LM'];
        ...
        // Nếu EM đã approved và chưa có LM approval -> cần LM
        if (emApproval.Decision === 'Approved' && !lmApproval) {
            return 'LM';
        }
        
        // Nếu EM đã rejected -> không cần tiếp
        if (emApproval.Decision === 'Rejected') {
            return null;
        }
        
        // Nếu cả EM và LM đã approved -> không cần tiếp
        if (emApproval.Decision === 'Approved' && lmApproval && lmApproval.Decision === 'Approved') {
            return null;
        }
    ...
    renderApprovalHistory(approvals) {
        ...
        html += `
        <div class="approval-item ${emApproval ? (emApproval.Decision === 'Approved' ? 'approved' : 'rejected') : 'pending'}">
        ...
        html += `
        <div class="approval-item ${lmApproval ? (lmApproval.Decision === 'Approved' ? 'approved' : 'rejected') : (emApproval && emApproval.Decision === 'Approved' ? 'pending' : 'blocked')}">
            <div class="approval-role flex items-center gap-2">
    ...
    async submitApproval() {
        ...
        const role = document.getElementById('approverRole')?.value;
        const decision = document.querySelector('input[name="decision"]:checked')?.value;
        const comment = document.getElementById('approvalComment')?.value.trim();
        
        if (!role || !decision) {
            this.showAlert('Vui lòng chọn vai trò và quyết định phê duyệt', 'warning');
            return;
        }
        ...
        const nextRole = this.determineNextApprovalRole();
        if (role !== nextRole) {
            this.showAlert(`Không thể phê duyệt với vai trò ${role}. Vai trò tiếp theo cần phê duyệt là ${nextRole}`, 'warning');
            return;
        }
        ...
        try {
            const response = await fetch(`/repair_requests/${this.currentRequestId}/approve`, {
                method: 'POST',
                headers: {
            ...
            if (decision === 'Approved' && role === 'LM') {
                message = 'Yêu cầu đã được phê duyệt hoàn toàn và chuyển sang trạng thái báo giá';
    ...
    renderPagination() {
        ...
        const totalPages = Math.ceil(this.currentRequests.length / this.itemsPerPage);
        ...
        let html = `
            <div class="text-gray-400 text-sm">
                Hiển thị <span class="text-white font-semibold">${Math.min(this.itemsPerPage, this.currentRequests.length)}</span> 
                trong tổng số <span class="text-white font-semibold">${this.currentRequests.length}</span> yêu cầu
            </div>
            <div class="flex items-center gap-2">
        `;
        ...
        // Previous button
        html += `
            <button onclick="approveManager.changePage(${this.currentPage - 1})" 
            ...
            if (i === 1 || i === totalPages || (i >= this.currentPage - 1 && i <= this.currentPage + 1)) {
                html += `
                    <button onclick="approveManager.changePage(${i})" 
                            class="px-3 py-2 rounded-lg ${i === this.currentPage ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'} transition">
                        ${i}
                    </button>
                `;
    ...
    // Thay đổi trang
    changePage(page) {
        if (page < 1 || page > Math.ceil(this.currentRequests.length / this.itemsPerPage)) return;
    ...
    showAlert(message, type) {
        ...
        const typeConfig = {
            success: { bg: 'bg-green-900/90', text: 'text-green-300', icon: 'fa-check-circle' },
            warning: { bg: 'bg-yellow-900/90', text: 'text-yellow-300', icon: 'fa-exclamation-triangle' },
            danger: { bg: 'bg-red-900/90', text: 'text-red-300', icon: 'fa-exclamation-circle' },
            info: { bg: 'bg-blue-900/90', text: 'text-blue-300', icon: 'fa-info-circle' }
        };
...
document.addEventListener('DOMContentLoaded', () => {
    approveManager = new ApproveRequestManager();
});